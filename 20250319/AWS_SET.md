...
在我以前的工作中，我做了很多事情。我领导一个测试团队，制定测试计划。给我的团队分配任务。计划软件发布。编写测试脚本。制作测试报告。确保软件测试良好。我了解自动化和测试。我了解 Postman 和 Jmeter 等工具。我了解 Java、Python、Vue、React 和 SQL。我了解数据库、Linux 和 Docker。我使用 Git 进行版本控制。我有 PMP 认证。我可以很好地与团队合作。
...
At my old job, I did many things. I led a test team, made test plans. gave tasks to my team. planned software releases. wrote test scripts. made test reports. made sure the software was tested well. I know automation and testing. And tools like Postman and Jmeter. I know Java, Python, Vue, React, and SQL. I know databases Linux and Docker. I can use Git for version control. I have a PMP certification. I can work well with a team.



#### 为什么要离职？

1. There is no salary increase opportunity at AsiaInfo, absolutely no 亚信科技没有调薪机会，完全没有
...
>
> ![image-20250303124558013](.\MarkdownImageUpload\image-20250303124558013.png)
...
> **Q：全局变量和局部变量变量名能否一样？**
>
> 是的，全局变量和局部变量的变量名可以一样。当全局变量和局部变量名称相同时，在局部作用域中，会优先使用局部变量
>
> 
>
> **Q：Python 里 is 和 == 的区别？**
>
> `is` 和 `==` 在 Python 中有明显的区别：
>
> - `==` 比较的是两个对象的值是否相等（值比较）
> - `is` 比较的是两个对象是否是同一个对象，即它们的内存地址是否相同（身份比较）
>
> 
>
> **Q：Python 变量的创建与消亡过程**
...
> 变量创建过程
>
> 1. 变量声明和赋值： 当你写 x = 10时，Python 实际上做了两件事：
>    - 创建一个整数对象 10
>    - 将变量名 x 与这个对象绑定（即变量成为对象的引用）
> 2. **引用计数增加**： 每个 Python 对象都有一个引用计数器，当变量绑定到对象时，对象的引用计数加 1
> 3. **命名空间记录**： 变量名被记录在当前的命名空间中（可能是局部、全局或内置命名空间）
>
> 变量消亡过程
>
> 1. **作用域结束**： 当包含变量的作用域结束时（如函数执行完毕），局部变量就会被销毁
> 2. **引用解除**： 变量与对象之间的绑定被解除，对象的引用计数减 1
> 3. 垃圾回收：
>    - **引用计数为零**：如果对象的引用计数降为 0，Python 会立即回收该对象的内存
>    - **循环引用**：如果存在循环引用，Python 的垃圾收集器会定期运行来检测并回收这些对象
>
> 
>
> **Q：深拷贝浅拷贝的区别是什么？**
...
> 浅拷贝（Shallow Copy）
>
> - 创建一个新对象，但是它包含的是原始对象中元素的引用
> - 只复制最外层的容器，内部嵌套的对象仍然共享引用
> - 可以通过`copy.copy()`、`list.copy()`、切片操作`[:]`、`dict.copy()`等方式创建
>
> 深拷贝（Deep Copy）
>
> - 创建一个新对象，并且递归地复制原始对象中的所有嵌套对象
> - 原始对象和复制对象完全独立，不共享任何子对象
> - 通过`copy.deepcopy()`实现
>
> 
>
> **Q：什么是协程？**
>
> 协程(Coroutines)是一种特殊的函数，它可以在执行过程中暂停并稍后恢复，允许非抢占式的多任务处理。这与传统函数不同，传统函数一旦开始执行就会一直运行到完成。
>
> 在Python中，协程主要通过`async/await`语法实现。以下是协程的主要特点：
>
> 协程的关键特性
>
> 1. **可挂起和恢复**：协程可以在执行过程中的特定点暂停，并在之后从暂停点继续执行
> 2. **非抢占式**：协程自己决定何时让出控制权，而不是被操作系统强制中断
> 3. **轻量级**：比线程消耗更少的资源
> 4. **协作式多任务**：多个协程可以协作完成任务，尤其适合I/O密集型应用
>
> 
>
