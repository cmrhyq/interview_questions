...
1. 我首先会看是不是需求中没有明确规定要实现的，这种我就会找产品并发邮件去确认是否需要完善或者更改需求，商量好之后是否需要更改，然后就是后续看看我的测试计划会不会受影响之类的问题
2. 这是一个bug，但是有可能开发说的不是的意思是很少有人这么去操作，这样的话我首先会提交bug，再和开发再去沟通并复现这个问题，然后确认这个问题是否需要修改，如果不修改，则需要在此bug中加上不修改的原因是什么。
